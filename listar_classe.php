		...
		<tbody>
			<?php while($linhas = mysqli_fetch_assoc($resultado)){ ?>
								<tr>
										...
										<td>
											<?php 
															$numclasse =  $linhas['id_classes'];
															$resultado_=mysqli_query($conn, "SELECT * FROM usuarios WHERE classe = $numclasse ORDER BY nome");
															$linhas_=mysqli_num_rows($resultado_);
															$contador = 0;

																	while($linhas_ = mysqli_fetch_assoc($resultado_)){
																		$contador ++;

																		echo "<b>"."$contador"." - "."</b>".$linhas_['nome']; 
																		echo "<br>";
																	}
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<form method="POST" action="processa/editar_classe.php" enctype="multipart/form-data" autocomplete="off">
							...
							<div class="form-group">
								<label for="recipient-secretario" class="control-label">Secretario:</label>
								<input name="secretario" type="text" class="form-control" id="recipient-secretario">
							</div>
