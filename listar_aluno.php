...
session_start();
	...
	$resultado=mysqli_query($conn, "SELECT * FROM usuarios ORDER BY classe, nome");
	$linhas=mysqli_num_rows($resultado);

	$conn_qtd_alunos = mysqli_connect($servidor, $usuario, $senha, $dbname);
	$resultado_qtd_alunos=mysqli_query($conn_qtd_alunos, "SELECT * FROM usuarios");
...
<div class="container theme-showcase" role="main">      
  <div class="page-header">
	...
	<?php //VRF quantos alunos tem nas classes
		...
		$sql = mysqli_query($conn_qtd_alunos, "SELECT *, COUNT(niveis_acesso_id) qtd_alunos FROM usuarios WHERE niveis_acesso_id <=4");
				while ($dados = mysqli_fetch_array($sql)){
					extract($dados); }
					echo "(Total: $qtd_alunos)";
					?>
					<a href="cad_usuario_aluno.php"><img src="imagens/addv.png"></a><a href="imprimir_alunos.php" target="_blank"><img src="imagens/printer.png"></a></h1>
  </div>

    <form class="form-horizontal" method="POST" action="pesquisa_aluno.php" autocomplete="off">
    	<div class="form-group">
      			<label for="inputEmail3" class="col-sm-2 control-label">Pesquisar Nome</label>
      		<div class="col-sm-10">
      			<input type="text" class="form-control" name="nome" autofocus="" required="" placeholder="Digite Partes do Nome - Exemplo: 'Slo' | Nao Usar Acentos"><br>
      			<input name="pesquisar" type="submit" value="Pesquisar" class="btn btn-success">
	      	</div>
      </div>
  	</form>
					...
					<div class="modal-body">
						<form method="POST" action="processa/editar_usuario_aluno.php" enctype="multipart/form-data" autocomplete="off">
							...
							<div class="form-group">
								...
								<input name="niveis_acesso_id" type="text" class="form-control" id="recipient-niveis_acesso_id" placeholder="1 - 2 - 3">
							</div>
