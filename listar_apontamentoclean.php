...
<?php
  session_start();
	...
	$resultado=mysqli_query($conn, "SELECT * FROM apontamento ORDER BY id_apontamento DESC");
		...
		<tbody>
			<?php while($linhas = mysqli_fetch_assoc($resultado)){ ?>
								...
								<!--	<td><?php echo $linhas['id_apontamento']; ?></td>-->
									...
									<?php //MAIOR DATA
									$linhas = $linhas ['dataapontamento'];
											$datadia = $linhas[6].$linhas[7];
											$datames = $linhas[4].$linhas[5];
											$dataano = $linhas[0].$linhas[1].$linhas[2].$linhas[3];?>
